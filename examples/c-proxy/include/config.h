...
private:
	...
	config create_config()
	{
		...
		std::string colors;
		colors.assign(_arg_map[COLORS]);
		std::stringstream colors_s(colors);
		while (getline(colors_s, item, ','))
			ret.colors.push_back((uint32_t)atoi(item.c_str()));
