...
void ycsb_insert::Run() {
        ...
        m_map->put(key, value, m_color, m_dep_color, m_is_causal);
...
void ycsb_insert::AsyncRun() {
        ...
        m_map->async_put(key, value, m_color, m_dep_color, m_is_causal);
}
                ...
                for (auto j = 0; j < proportions.size(); j++) {
                        if (proportions[j] > r && allocations[j] > 0) { 
                                if ((*m_workload)[j].op_type == "get") {
                                        ...
                                        txns[i] = new ycsb_insert(m_map, &(*m_workload)[j].color, (*m_workload)[j].has_dependency ? &(*m_workload)[j].dep_color : NULL, 0, m_range, m_context, Txn::optype::PUT, (*m_workload)[j].is_causal);
                                }
